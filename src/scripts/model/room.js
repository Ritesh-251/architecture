...
export const defaultRoomTexture = {url: 'rooms/textures/hardwood.png', scale: 400, color: '#cccccc'};
...
export class Room extends EventDispatcher
{
	...
	setRoomWallsTexture(textureUrl, textureStretch, textureScale, textureColor)
	{
		var edge = this.edgePointer;
		var iterateWhile = true;
		edge.setTexture(textureUrl, textureStretch, textureScale, textureColor);
		while (iterateWhile)
		{
	...
	{
		var uuid = this.getUuid();
		var texture = {url: textureUrl, stretch: textureStretch, scale: textureScale, color: textureColor || '#cccccc'};
		this.floorplan.setFloorTexture(uuid, texture.url, texture.scale, texture.color);
